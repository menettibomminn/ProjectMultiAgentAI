"""Auth Agent — main orchestrator.

Single-run entrypoint: reads one task from inbox, validates it, generates a
token management proposal report, writes audit log, updates HEALTH.md, and exits.

This agent NEVER handles actual tokens. It produces proposed_changes only.
"""
...
from Agents.auth_agent.lock_manager import LockManager, LockError
from Agents.auth_agent.logger import get_logger
from Agents.auth_agent.auth_audit_logger import write_audit_entry
from Agents.auth_agent.auth_report_generator import (
    generate_error_report,
    generate_report,
    write_report,
)
from Agents.auth_agent.auth_task_parser import parse_task_file


class AuthAgent:
    """Auth agent that processes one task per invocation."""

    def __init__(self, config: AuthAgentConfig | None = None) -> None:
        self.config = config or AuthAgentConfig.from_env()
    ...
    def run_once(self) -> bool:
        ...
        try:
            ...
            # Lock resource is the user being acted upon
            auth_req = task["auth_request"]
            resource_id = auth_req.get("target_user_id", user_id)
    ...
    def _update_health(
        ...
        entry = (
            ...
            f"| notes | auto-updated by auth_agent.py |\n"
        )

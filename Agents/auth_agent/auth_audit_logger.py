"""Write structured audit log entries for every auth agent execution.

Each run produces one file: ops/audit/auth-agent/{timestamp}.json
Content includes checksums, timestamps, operation details, and runtime metrics.

SECURITY:
- Actual token values are NEVER included in audit entries.
- Only metadata is logged: user_id, scopes, operation, expiry.
"""

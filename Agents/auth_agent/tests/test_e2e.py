"""End-to-end integration tests for the auth agent pipeline."""
...
class TestFullPipeline:
    ...
    def test_full_pipeline(
        self, test_config: AuthAgentConfig, task_file: Path
    ) -> None:
        agent = AuthAgent(test_config)
        ...
        assert "auth-042" in health_text
        assert "healthy" in health_text

    def test_no_task_returns_false(
        self, test_config: AuthAgentConfig
    ) -> None:
        agent = AuthAgent(test_config)
        result = agent.run_once()

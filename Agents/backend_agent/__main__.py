"""Entry point for running the backend agent as a module.

Usage:
    python -m Agents.backend_agent --run-once
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backend Agent")
    parser.add_argument(
        "--run-once",
        action="store_true",
    ...
    config = BackendAgentConfig.from_env()
    if args.agent_id:

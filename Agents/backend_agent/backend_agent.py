"""Backend Agent — main orchestrator.

Single-run entrypoint: reads one task from inbox, validates it, generates a
backend processing proposal report, writes audit log, updates HEALTH.md, and exits.
"""
...
from Agents.backend_agent.lock_manager import LockManager, LockError
from Agents.backend_agent.logger import get_logger
from Agents.backend_agent.backend_audit_logger import write_audit_entry
from Agents.backend_agent.backend_report_generator import (
    generate_error_report,
    generate_report,
    write_report,
)
from Agents.backend_agent.backend_task_parser import parse_task_file


class BackendAgent:
    """Backend agent that processes one task per invocation."""

    def __init__(self, config: BackendAgentConfig | None = None) -> None:
        self.config = config or BackendAgentConfig.from_env()
    ...
    def run_once(self) -> bool:
        ...
        try:
            ...
            # Lock resource: sheet_id when present, otherwise task_id
            req = task["request"]
            resource_id = req.get("sheet_id") or task_id
    ...
    def _update_health(
        ...
        entry = (
            ...
            f"| notes | auto-updated by backend_agent.py |\n"
        )

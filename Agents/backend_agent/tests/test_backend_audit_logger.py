"""Tests for backend_audit_logger."""
...
from Agents.backend_agent.backend_audit_logger import (
    compute_checksum,
    write_audit_entry,
)
...
class TestWriteAuditEntry:
    ...
    def _base_kwargs(self, audit_dir: Path) -> dict[str, Any]:
        return {
            ...
            "report": {"status": "success", "task_id": "backend-042"},
        }
    ...
    def test_creates_file(self, tmp_path: Path) -> None:
        ...
        assert data["task_id"] == "backend-042"
    ...
    def test_filename_format(self, tmp_path: Path) -> None:
        path = write_audit_entry(**self._base_kwargs(tmp_path))
        assert path.name.endswith("_backend-042.json")

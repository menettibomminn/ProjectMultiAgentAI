"""End-to-end integration tests for the frontend agent pipeline."""
...
class TestFullPipeline:
    ...
    def test_full_pipeline(
        self, test_config: FrontendAgentConfig, task_file: Path
    ) -> None:
        agent = FrontendAgent(test_config)
        ...
        assert "fe-001" in health_text
        assert "healthy" in health_text

    def test_no_task_returns_false(
        self, test_config: FrontendAgentConfig
    ) -> None:
        agent = FrontendAgent(test_config)
        result = agent.run_once()

"""Tests for frontend_audit_logger."""
...
from Agents.frontend_agent.frontend_audit_logger import (
    compute_checksum,
    write_audit_entry,
)
...
class TestWriteAuditEntry:
    ...
    def _base_kwargs(self, audit_dir: Path) -> dict[str, Any]:
        return {
            ...
            "report": {"status": "success", "task_id": "fe-001"},
        }
    ...
    def test_no_sheets_data_in_audit(self, tmp_path: Path) -> None:
        """Verify no raw sheet data leaks into audit."""
        kwargs = self._base_kwargs(tmp_path)
        path = write_audit_entry(**kwargs)
    ...
    def test_filename_format(self, tmp_path: Path) -> None:
        path = write_audit_entry(**self._base_kwargs(tmp_path))
        assert path.name.endswith("_fe-001.json")

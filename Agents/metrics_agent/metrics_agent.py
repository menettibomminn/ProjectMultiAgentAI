"""Metrics Agent — main orchestrator.

Single-run entrypoint: reads one task from inbox, validates it, generates a
metrics analysis report, writes audit log, updates HEALTH.md, and exits.
"""
...
from Agents.metrics_agent.lock_manager import LockManager, LockError
from Agents.metrics_agent.logger import get_logger
from Agents.metrics_agent.metrics_audit_logger import write_audit_entry
from Agents.metrics_agent.metrics_report_generator import (
    generate_error_report,
    generate_report,
    write_report,
)
from Agents.metrics_agent.metrics_task_parser import parse_task_file


class MetricsAgent:
    """Metrics agent that processes one task per invocation."""

    def __init__(self, config: MetricsAgentConfig | None = None) -> None:
        self.config = config or MetricsAgentConfig.from_env()
    ...
    def run_once(self) -> bool:
        ...
        try:
            ...
            # Lock resource is the target_team_id or target_agent_id
            metrics_req = task["metrics_request"]
            resource_id = (
                metrics_req.get("target_team_id")
                or metrics_req.get("target_agent_id")
                or team_id
            )
    ...
    def _update_health(
        ...
        entry = (
            ...
            f"| notes | auto-updated by metrics_agent.py |\n"
        )

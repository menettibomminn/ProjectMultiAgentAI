"""End-to-end integration tests for the metrics agent pipeline."""
...
class TestFullPipeline:
    ...
    def test_full_pipeline(
        self, test_config: MetricsAgentConfig, task_file: Path
    ) -> None:
        agent = MetricsAgent(test_config)
        ...
        assert "metrics-001" in health_text
        assert "healthy" in health_text

    def test_no_task_returns_false(
        self, test_config: MetricsAgentConfig
    ) -> None:
        agent = MetricsAgent(test_config)
        result = agent.run_once()
